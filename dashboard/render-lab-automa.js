/* render-lab-automa.js — Quiz de Laboratorio · Automatización de Procesos y Control Distribuido (IFA00117)
...
(function(){
  const QUICES = [
    {id:"quiz1",nombre:"Quiz 1 · Práctica 01"},
    {id:"quiz2",nombre:"Quiz 2 · Práctica 02"},
    {id:"quiz3",nombre:"Quiz 3 · Práctica 03"},
    {id:"quiz4",nombre:"Quiz 4 · Práctica 04"}
  ];
  const BLOQUES = {
  ...
  function todos(){
    const d=(typeof DATA!=="undefined" && DATA.labAutoma) ? DATA.labAutoma : {};
  ...
  function render(){
    const cont=document.getElementById("vista-labAutoma"); if(!cont) return;
    ...
    const head=`<div class="seccion-head"><h2><i class="fa-solid fa-gears"></i> Quiz de Laboratorio · Automatización de Procesos</h2>
        <select id="lab-au-seg" style="${selSty}"><option value="general">General · todos los quices</option>
    ...
    cont.innerHTML=`
      ...
      <div class="charts-row">
        <div class="seccion"><h2><i class="fa-solid fa-chart-column"></i> Distribución de calificaciones</h2>
          ...
          <div class="chart-wrap"><canvas id="lab-au-dist"></canvas></div></div>
        <div class="seccion"><h2><i class="fa-solid fa-chart-pie"></i> Aprobación</h2>
          ...
          <div class="chart-wrap"><canvas id="lab-au-aprob"></canvas></div></div>
      </div>
      <div class="seccion"><h2><i class="fa-solid fa-layer-group"></i> Desempeño por bloque temático</h2>
        ...
        <div class="chart-wrap" style="height:${Math.max(220,blq.length*40)}px"><canvas id="lab-au-blq"></canvas></div>
      ...
      <div class="seccion"><h2><i class="fa-solid fa-code-compare"></i> Comparativo por quiz</h2>
        <div class="chart-wrap" style="height:280px"><canvas id="lab-au-comp"></canvas></div>
        <div style="overflow-x:auto;margin-top:14px"><table>
      ...
      <div class="seccion"><h2><i class="fa-solid fa-arrow-trend-down"></i> Temas por reforzar · orientación</h2>
        ...
        <div class="chart-wrap" style="height:${Math.max(220,temas.length*24)}px"><canvas id="lab-au-temas"></canvas></div>
    ...
    crearChart("lab-au-temas","bar",
      {labels:temas.map(t=>t.tema),datasets:[{data:temas.map(t=>t.pct),backgroundColor:temas.map(t=>tokPct(t.pct))}]},
      barXpct);
  }

  function bindSel(){
    const s=document.getElementById("lab-au-seg"); if(s){ s.value=seg; s.onchange=e=>{seg=e.target.value;render();}; }
  }
  window.renderLabAutoma = render;
})();
