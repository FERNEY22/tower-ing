/* render-lab-automa-v2.js — Quiz de Laboratorio · Automatización de Procesos · Versión 2 y Control Distribuido (IFA00117)
   VERSIÓN 2 del banco de preguntas. Misma estructura y misma analítica que render-lab-automa.js;
   lee la porción de quiz_lab_automa cuyos intentos corresponden a los ids quiz1v2…quiz4v2.
...
(function(){
  const QUICES = [
    {id:"quiz1v2",nombre:"Quiz 1 · Práctica 01 (v2)"},
    {id:"quiz2v2",nombre:"Quiz 2 · Práctica 02 (v2)"},
    {id:"quiz3v2",nombre:"Quiz 3 · Práctica 03 (v2)"},
    {id:"quiz4v2",nombre:"Quiz 4 · Práctica 04 (v2)"}
  ];
  const BLOQUES = {
    A:"Objetivos y normas de seguridad",
    B:"Banco de microneumática",
    C:"Tablero eléctrico",
    D:"PLC",
    E:"Ficha de reconocimiento",
    F:"Cableado y circuitos"
  };
  const ORDEN_BLQ = ["A","B","C","D","E","F"];
  ...
  function todos(){
    const d=(typeof DATA!=="undefined" && DATA.labAutomaV2) ? DATA.labAutomaV2 : {};
  ...
  function render(){
    const cont=document.getElementById("vista-labAutomaV2"); if(!cont) return;
    ...
    const head=`<div class="seccion-head"><h2><i class="fa-solid fa-gears"></i> Quiz de Laboratorio · Automatización de Procesos · Versión 2</h2>
        <select id="lab-au2-seg" style="${selSty}"><option value="general">General · todos los quices</option>
    ...
    cont.innerHTML=`
      ...
      <div class="charts-row">
        <div class="seccion"><h2><i class="fa-solid fa-chart-column"></i> Distribución de calificaciones</h2>
          ...
          <div class="chart-wrap"><canvas id="lab-au2-dist"></canvas></div></div>
        <div class="seccion"><h2><i class="fa-solid fa-chart-pie"></i> Aprobación</h2>
          ...
          <div class="chart-wrap"><canvas id="lab-au2-aprob"></canvas></div></div>
      </div>
      <div class="seccion"><h2><i class="fa-solid fa-layer-group"></i> Desempeño por bloque temático</h2>
        <p style="color:var(--ink-3);font-size:var(--t-sm);margin:-6px 0 12px">% de acierto en los ítems de cada bloque de la guía. 🔴 &lt;50% reforzar · 🟠 50–74% revisar · 🟢 ≥75% dominado.</p>
        <div class="chart-wrap" style="height:${Math.max(220,blq.length*40)}px"><canvas id="lab-au2-blq"></canvas></div>
      ...
      <div class="seccion"><h2><i class="fa-solid fa-code-compare"></i> Comparativo por quiz</h2>
        <div class="chart-wrap" style="height:280px"><canvas id="lab-au2-comp"></canvas></div>
        <div style="overflow-x:auto;margin-top:14px"><table>
      ...
      <div class="seccion"><h2><i class="fa-solid fa-arrow-trend-down"></i> Temas por reforzar · orientación</h2>
        ...
        <div class="chart-wrap" style="height:${Math.max(220,temas.length*24)}px"><canvas id="lab-au2-temas"></canvas></div>
        ${debiles.length?`<div class="grupos-info" style="margin-top:16px">
          ...
          Retoma estos puntos de la guía antes de la siguiente práctica.</div>`:""}
      </div>
    ...
    crearChart("lab-au2-temas","bar",
      {labels:temas.map(t=>t.tema),datasets:[{data:temas.map(t=>t.pct),backgroundColor:temas.map(t=>tokPct(t.pct))}]},
      barXpct);
  }

  function bindSel(){
    const s=document.getElementById("lab-au2-seg"); if(s){ s.value=seg; s.onchange=e=>{seg=e.target.value;render();}; }
  }
  window.renderLabAutomaV2 = render;
})();
