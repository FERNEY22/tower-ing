/* render-lab-electro-v2.js — Quiz de Laboratorio · Electrónica · Versión 2 (IFA00112)
   VERSIÓN 2 del banco de preguntas. Misma estructura y misma analítica que render-lab-electro.js;
   lee la porción de quiz_lab_electro cuyos intentos corresponden a los ids quiz1v2…quiz4v2.
...
(function(){
  const QUICES = [
    {id:"quiz1v2",nombre:"Quiz 1 · Instrumentos y Práctica No 1 (v2)"},
    {id:"quiz2v2",nombre:"Quiz 2 · Práctica No 2 (v2)"},
    {id:"quiz3v2",nombre:"Quiz 3 · Práctica No 3 (v2)"},
    {id:"quiz4v2",nombre:"Quiz 4 · Práctica No 4 (v2)"}
  ];
  const BLOQUES = {
  ...
  function todos(){
    const d=(typeof DATA!=="undefined" && DATA.labElectroV2) ? DATA.labElectroV2 : {};
  ...
  function render(){
    const cont=document.getElementById("vista-labElectroV2"); if(!cont) return;
    ...
    const head=`<div class="seccion-head"><h2><i class="fa-solid fa-wave-square"></i> Quiz de Laboratorio · Electrónica · Versión 2</h2>
        <select id="lab-el2-seg" style="${selSty}"><option value="general">General · todos los quices</option>
    ...
    cont.innerHTML=`
      ...
      <div class="charts-row">
        <div class="seccion"><h2><i class="fa-solid fa-chart-column"></i> Distribución de calificaciones</h2>
          ...
          <div class="chart-wrap"><canvas id="lab-el2-dist"></canvas></div></div>
        <div class="seccion"><h2><i class="fa-solid fa-chart-pie"></i> Aprobación</h2>
          ...
          <div class="chart-wrap"><canvas id="lab-el2-aprob"></canvas></div></div>
      </div>
      <div class="seccion"><h2><i class="fa-solid fa-layer-group"></i> Desempeño por bloque temático</h2>
        ...
        <div class="chart-wrap" style="height:${Math.max(220,blq.length*40)}px"><canvas id="lab-el2-blq"></canvas></div>
      ...
      <div class="seccion"><h2><i class="fa-solid fa-code-compare"></i> Comparativo por quiz</h2>
        <div class="chart-wrap" style="height:280px"><canvas id="lab-el2-comp"></canvas></div>
        <div style="overflow-x:auto;margin-top:14px"><table>
      ...
      <div class="seccion"><h2><i class="fa-solid fa-arrow-trend-down"></i> Temas por reforzar · orientación</h2>
        ...
        <div class="chart-wrap" style="height:${Math.max(220,temas.length*24)}px"><canvas id="lab-el2-temas"></canvas></div>
    ...
    crearChart("lab-el2-temas","bar",
      {labels:temas.map(t=>t.tema),datasets:[{data:temas.map(t=>t.pct),backgroundColor:temas.map(t=>tokPct(t.pct))}]},
      barXpct);
  }

  function bindSel(){
    const s=document.getElementById("lab-el2-seg"); if(s){ s.value=seg; s.onchange=e=>{seg=e.target.value;render();}; }
  }
  window.renderLabElectroV2 = render;
})();
